...
def safe_input(prompt=""):
    # 清空 stdin 緩衝區 (避免 Windows 下 subprocess 繼承殘留的 Enter 鍵)
    try:
        import msvcrt
        while msvcrt.kbhit():
            msvcrt.getch()
...
def get_selected_news():
    """撈取「已選定 (is_selected=1)」且「未處理 (is_processed=0)」的新聞"""
...
def run(mode="standard", model=None, log=print):
    """
    針對所有已選定 (is_selected=1) 的新聞，逐則呼叫 Gemini 產製短影音腳本。
    mode: "standard" (30秒) 或 "pro" (60秒)
    model: Gemini 模型 ID，None 則採用該模式清單的預設 (第一個) 模型
    回傳：{"success": 成功筆數, "failed": 失敗筆數, "outputs": [...]}
    """
    is_pro_mode = (mode == "pro")

    if model is None:
        default_models = config.SCRIPT_MODELS_PRO if is_pro_mode else config.SCRIPT_MODELS_STANDARD
        model = next(iter(default_models.values()))
    target_model = model
    ...
    selected_news = get_selected_news()

    if not selected_news:
        log("目前沒有待處理的「已選定」新聞。請先執行 step1_5_selector.py 進行選題！")
        conn.close()
    ...
    for news in selected_news:
       ...
       - 第 4 個提示詞：針對結尾或展望未來的畫面設計。"""
        ...
        try:
            ...
            log(f"✅ 生成成功！({len(final_script)} 字) -> {txt_path} & .json")
            log(f"【🎙️ 預覽】：{final_script[:50]}...")

            # 5. 更新資料庫狀態 (標記已處理並重置選定狀態)
            cursor.execute("UPDATE DailyNews SET is_processed = 1, is_selected = 0 WHERE id = ?", (db_id,))
            success_count += 1
            outputs.append({"id": db_id, "txt_path": txt_path, "json_path": json_path, "script": script_data})

        except Exception as e:
            log(f"❌ [ID: {db_id}] 腳本生成失敗: {e}")
            failed_count += 1
...
def main():
    ...
    mode_choice = safe_input("請輸入選項 (1 或 2，預設為 1): ").strip()
    ...
    if is_pro_mode:
        ...
        model_choice = safe_input("請輸入選項 (1, 2, 3 或 4，預設為 1): ").strip()
        if model_choice == "2":
            target_model = 'gemini-3-flash-preview'
        elif model_choice == "3":
        ...
        model_choice = safe_input("請輸入選項 (1 或 2，預設為 1): ").strip()
        if model_choice == "2":
            target_model = 'gemini-3.5-flash'
        else:
            target_model = 'gemini-3-flash-preview'

    run(mode=mode, model=target_model)
