...
#  核心功能二：RVC 音色轉換
# ============================================================
def apply_rvc_conversion(input_wav: str, output_wav: str, log=print):
    """
    ...
    log(f"      [RVC] 啟動轉換程序: 模型={RVC_MODEL_NAME}, 音調偏移={RVC_F0_UP_KEY}")
    try:
        # 第一階段嘗試：硬體加速
        subprocess.run(cmd, cwd=rvc_dir, check=True)
    except subprocess.CalledProcessError as e:
        log(f"      ⚠️ 硬體加速 (DML) 失敗或缺乏支援，將嘗試切換至 CPU 模型運算...")
        try:
            # 拔除 --dml 參數
            if "--dml" in cmd:
                cmd.remove("--dml")
            ...
            log(f"      ❌ RVC 轉換雙重失敗，將回退至 edge-tts 原始音源。詳細錯誤: {e2}")
            import shutil
...
#  主流程
# ============================================================
async def run(log=print):
    """讀取 output_scripts/ 內所有腳本，依序執行 edge-tts 生成與 RVC 音色轉換"""
    log("=" * 50)
    log(f"  🎙️  語音渲染中心 - 模式：【{VOICE_MODE.upper()}】")
    log("=" * 50)
    ...
    for script_path in script_files:
        ...
        if not script_text:
            log(f"  ⚠️  [{base_name}] 腳本內容為空，跳過。")
            continue

        log(f"🎬 正在處理：{base_name}")
        log(f"   📝 腳本預覽：{script_text[:40]}...")
        ...
        log(f"   🔊 [Step A] edge-tts 生成基礎音檔與字幕時間軸...")
        try:
            await generate_tts_audio(script_text, tts_wav_path, subs_json_path)
            log(f"   ✅ 基礎音檔與時間軸已儲存：{tts_wav_path} & .json")
        except Exception as e:
            log(f"   ❌ edge-tts 生成失敗：{e}")
            failed_count += 1
            continue
        ...
        if VOICE_MODE == "rvc":
            log(f"   🎨 [Step B] RVC 音色轉換 (蔡小花)...")
            try:
                apply_rvc_conversion(tts_wav_path, final_wav_path, log=log)
                log(f"   ✅ 蔡小花音色音檔已儲存：{final_wav_path}")
            except Exception as e:
                log(f"   ❌ RVC 轉換失敗：{e}")
        else:
            ...
            log(f"   ✅ [edge-tts 模式] 最終音檔：{final_wav_path}")

        success_count += 1
        outputs.append({"base_name": base_name, "final_wav": final_wav_path, "subs_json": subs_json_path})
        log("")

    log("🎉 所有腳本語音渲染完畢！")
    log(f"   輸出位置：{os.path.abspath(VOICES_DIR)}/")
    log("")
    if VOICE_MODE == "edge-tts":
        log("💡 提示：待蔡小花聲音模型訓練完成後，")
        log("         請填寫腳本上方的 RVC_MODEL_PATH 等設定，")
        log("         並將 VOICE_MODE 改為 'rvc' 即可切換至專屬音色！")
