...
| **圖形化介面** | **NiceGUI**（網頁式控制台，背景執行緒 + 即時 Log 廣播） |
...
| GUI | `app.py` + `gui/` | NiceGUI 圖形化控制台（儀表板 + 9 個模組頁面） |
...
除了逐一執行終端機指令，專案也提供一個以 **NiceGUI** 打造的網頁控制台，可視覺化管理整條管線：

```powershell
python app.py
```

啟動後會自動開啟瀏覽器（預設 `http://localhost:8080`），介面包含：

* **管線儀表板**：以卡片呈現每則新聞在「選定 → 腳本 → 語音 → 配圖 → 影片 → 發布」六個階段的進度，並可一鍵跳轉到下一步該執行的模組。
* **九個模組頁面**：資料蒐集、選題台、腳本生成、語音渲染、配圖生成、影片合成、多平台發布、歸檔，皆可獨立操作並帶參數執行。
* **即時 Log 面板**：所有模組的執行輸出即時顯示於畫面底部，長時間任務（配音、剪輯、發布）於背景執行，不會卡住介面。
* **執行進度顯示**：頂部狀態列即時顯示已耗時／預估剩餘時間；筆數可預先得知時（如腳本生成、配圖生成）顯示「目前/總數」確定進度條，未知時顯示動態進度條，確保長時間任務「看得出還活著」。
* **付費 API 二次確認**：任何會呼叫 Gemini / Imagen 的按鈕（RSS 篩選、腳本生成、配圖生成、發布文案）皆標示 💰，點擊後彈窗列出本次確切呼叫次數，需明確確認才會執行，取消不產生任何費用；本機運算（語音、剪輯）與唯讀操作皆一鍵直接執行、無需確認。
* **人工即時介入**：多平台發布頁面提供「跳過 / 強制重試 / 重置等待」按鈕，取代原本的終端機鍵盤操作。
...
模組 1, 1.5, 2, 3, 5, 6, 8, 9 ✅ 已完成並串接｜圖形化控制台 ✅ 已完成｜模組 4（虛擬角色嘴型同步）🚧 規劃中（目前用靜態去背頭像）
